...
use crate::{g_subclasses::action_entry::ContextAction, utils::config::get_config};

use super::util::ContextUI;

pub fn make_context() -> (ContextUI, Revealer) {
    let max_heigth = get_config().map_or(60, |c| c.appearance.height - 200);
    ...
    if !get_config().map_or(false, |c| c.behavior.animate) {
